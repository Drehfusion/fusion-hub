	...
	<?php
		...
		while($row=mysqli_fetch_assoc($result))
		{
			...
			echo'<tr> <td>'.$row['name'].'</td>
				...
				<td><input type="checkbox" value="'.$row['original_number'].'" name="checkbox[]" />delete</td></tr>';
...
if(isset($_POST['delete']))
{
	...
	{
			$query="DELETE FROM registry where id_no='$delete'";
			$result=mysqli_query($connect,$query);
			if($result)
			{
				echo'<script>var a=confirm("deleted from the database!")
					if(a==true)
					{
						window.location.assign("delete-child.php");
					}
					else
					{
						window.location.assign("delete-child.php");
